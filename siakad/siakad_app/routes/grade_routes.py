...
def upsert_grade():
    ...
    try:
        ...
        db.session.commit()
        logger.info(
            f"Grade upserted: student={grade.student_id} subject={grade.subject_id}"
        )
...
def transcript(student_id: int):
    ...
    items = [
        g.to_dict(include_subject=True)
        for g in Grade.query.filter_by(student_id=student_id).all()
    ]
    avg = round(sum(i["final"] for i in items) /
                len(items), 2) if items else 0.0
...
def class_report():
    ...
    grades = (
        Grade.query.filter(Grade.student_id.in_(student_ids)).all()
        if student_ids
        else []
    )
    ...
    table = {}
    for g in grades:
        table.setdefault(g.student_id, {})[
            subject_codes.get(g.subject_id, str(g.subject_id))
        ] = g.final_score

    # If HTML requested explicitly
    if "text/html" in (request.headers.get("Accept") or ""):
        # Build a sorted set of subject codes present
        all_codes = sorted({code for row in table.values()
