...
from siakad_app.models import Student, Teacher, Subject, Grade
...
def avg_by_subject():
    ...
    rows = (
        db.session.query(
            Subject.code,
            Subject.name,
            func.round(func.avg((Grade.tugas + Grade.uts + Grade.uas) / 3.0), 2).label('avg_final')
        )
    ...
    return jsonify([
        {'code': code, 'name': name, 'average': float(avg) if avg is not None else 0.0}
