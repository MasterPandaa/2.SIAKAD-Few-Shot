...
def list_subjects():
    ...
    if q:
        like = f"%{q}%"
        query = query.filter(
            or_(Subject.name.ilike(like), Subject.code.ilike(like)))

    pagination = query.order_by(Subject.name.asc()).paginate(
...
def create_subject():
    try:
        ...
        if payload.get("teacher_id") and not db.session.get(
